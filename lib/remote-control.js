...
function getParkMessages() {
    return park.messages.map(function (m) { return ({
        month: m.month,
        day: m.day,
        type: m.type,
        text: m.text,
...
function doNetworkCommand(command) {
    ...
    if ((args = doesCommandMatch(command, [GET])) !== false && typeof args === 'string' && args.length > 0) {
        ...
        if (typeof args === 'string' && (args = doesCommandMatch(args, ['player'])) !== false) {
            var argsComponents = typeof args === 'string' ? args.split(' ', 2) : ['', ''];
            var player = getPlayerByHash(argsComponents[0]);
            var result = {
                result: player !== null
            };
            if (result.result) {
        ...
        var player = getPlayerByHash(args);
        var result = {
            result: player !== null
        };
        if (result.result) {
